...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='trackingmamba', choices=['trackingmamba'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='trackingmamba', help='yaml configure file name')
...
def evaluate_vit(model, template, search):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, template, search, [], []),
...
if __name__ == "__main__":
    ...
    args = parse_args()
    '''update cfg'''
    yaml_fname = '/media/yy/4T/wbb4/TrackingMamba-main/experiments/%s/%s.yaml' % (args.script, args.config)
    ...
    if args.script == "trackingmamba":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_trackingmamba
