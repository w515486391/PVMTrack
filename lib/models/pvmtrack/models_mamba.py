...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        if if_abs_pos_embed:
            ...
            self.pos_embed_zd = nn.Parameter(torch.zeros(1, 64, self.embed_dim))
    ...
    def forward_features(self, z, zd, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_abs_pos_embed:                  # True 
            ...
            zd = zd + self.pos_embed_zd

            prompt_z = torch.cat((zd, z), dim=1)
            # 测试改
            # prompt_z = prompt_z.repeat(4, 1, 1)

            x = torch.cat((prompt_z, x), dim=1)           # torch.Size([B, 320, 384])
    ...
    def forward_features1(self, z, zd, x, x24, inference_params=None, if_random_cls_token_position=False,
        ...
        if not self.if_bidirectional:  # True
            ...
            for i in range(len(self.layers1) // 2):
                ...
                hidden_states_f, residual_f = self.layers1[i * 2](
                    hidden_states, residual, inference_params=inference_params
                )
                hidden_states_b, residual_b = self.layers1[i * 2 + 1](
    ...
    def forward_features2(self, z, zd, x, x26, inference_params=None, if_random_cls_token_position=False,
                         if_random_token_rank=False):
        # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
        # with slight modifications to add the dist_token
        ...
        if not self.if_bidirectional:  # True
            ...
            for i in range(len(self.layers2) // 2):
                ...
                hidden_states_f, residual_f = self.layers2[i * 2](
                    hidden_states, residual, inference_params=inference_params
                )
                hidden_states_b, residual_b = self.layers2[i * 2 + 1](
                    hidden_states.flip([1]), None if residual == None else residual.flip([1]),
                    inference_params=inference_params
                )
                hidden_states = hidden_states_f + hidden_states_b.flip([1])
